...
export default function CartProvider({ children }) {
  ...
  function validateCart(cart) {
    if (cart && Array.isArray(cart.items) && typeof cart.totalPrice === 'number' && typeof cart.totalCount === 'number') {
      return cart;
    }
    return EMPTY_CART;
  }
  ...
  const changeQuantity = (cartItem, newQuantity) => {
    const { food } = cartItem;
    const changedCartItem = { ...cartItem, quantity: newQuantity, price: food.price * newQuantity };
